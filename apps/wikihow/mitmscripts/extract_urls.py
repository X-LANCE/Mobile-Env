...
#  Main Structure {{{ # 
with open("../flows/wikihow-20220820.flow", "rb") as fl_f:
    ...
    for f in flow_reader.stream():
        if isinstance(f, http.HTTPFlow) and f.request.pretty_host=="www.wikihow.com":
            ...
            url = f.request.path
            if url.startswith("/wikiHowTo?search=") or url.endswith("?wh_an=1&amp=1"):
                dump_name = url.replace("/", "%2f")
                #if len(dump_name)>100:
                    #dump_name = dump_name[:100]

                with open(os.path.join("pages/search-pages-2/header", dump_name + ".txt"), "w") as opt_f:
                    for k, v in f.response.headers.items():
                        opt_f.write("{:}: {:}\n".format(k, v))
                with open(os.path.join("pages/search-pages-2/html", dump_name + ".html.gz"), "wb") as opt_f:

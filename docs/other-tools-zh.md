...
网页界面工具保存下来的标注，每条交互历程会保存为一个pickle文件，名为`标注者名.%d.pkl`，其中`%d`位置是历程的编号。转存工具会将各单独的历程汇总，按其任务分别保存。其调用格式为

```sh
python resave_pickles.py PKL_NAME:S:T DUMPDIR TASKFOLDER
```

+ `PKL_NAME` - 要转存的历程文件路径的模式，如`web-dumps/zdy023.%d.pkl`
+ `S`、`T` - 历程编号的范围（包含），用来取代路径模式串中的`%d`
...
历程文件中的每条历程会提出一组帧，存放于`DUMPDIR`下的一个子文件夹中。该命令会依次打印出各历程的帧保存的路径。事实上，每条历程保存的子文件夹会命名为`任务定义文件名:历程序号%历程数量#历程长度`。
...
1. 先对第2条（序号从0记）历程应用`delete`修改，将区间`[1, 3)`的步骤删去（序号从0始）
2. 再对第2条历程应用`rewardize`修改，该修改要将第3步记录（应用过`delete`后的第3步，也就是最开始的第5步）的回报减去1。

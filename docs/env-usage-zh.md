...
然后选择系统镜像的版本。同样，镜像版本推荐采用Android 11 (API Level 30)。同时请选择Google APIs版本而非Google Play版本，这是由于，为解决部分联网应用的固定证书的问题，需要获取虚拟机的root权限，而在Google Play版本上，root权限会获取失败。
...
许多信息类应用依赖互联网获取动态内容，这些内容取决于使用应用的时间和位置，这会导致训练、测试智能体变得不一致，导致结果难以比较。这可以通过爬取一定量应用的在线内容并固定之，在训练、测试时通过中间人代理工具将保存的内容回放给应用程序来解决。但大多应用都使用HTTPS传输数据，因此需要应用端信任中间人代理的SSL证书，才能截取通信流量并正确回放内容。然而部分应用采用了固定证书的策略，这使得简单地将证书在操作系统设置中加入”用户证书“列表，不能解决问题。针对该问题，测试了三种解决方案，并提供了快捷配置的脚本，具体步骤请参考[针对动态信息应用的解决方案](dynamic-app-zh.md)。
...
* `mitm_config` - 指定中间人代理的配置。若需要中间人代理回放流量，则要提供一个词典参数，其中包含三个通用字段：
  ...
  - `method` - 指定解决方案，共支持三种方案
    - `syscert` - 伪装系统证书
    - `frida` - 采用注入工具[Frida](https://github.com/frida/frida)运行时替换证书验证器
    - `packpatch` - 修改软件包配置，以解除其固定证书
    具体内容请参考[针对动态信息应用的解决方案](dynamic-app-zh.md)；其中`frida`方案可以在词典中指定三个额外参数：
